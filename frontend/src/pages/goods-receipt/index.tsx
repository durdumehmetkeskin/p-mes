...
export function GoodsReceiptForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    try {
      await axiosInstance.post("/inventory-transactions/receive", {
        materialId,
        warehouseId,
      ...
      open?.({
        ...
        description: "Stock balance updated.",
      });
      reset();
      onSuccess?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
            ...
            <Button type="submit" disabled={submitting}>
              {submitting ? "Receiving..." : "Receive material"}
            </Button>
    </form>
  );
}
...
export const GoodsReceipt = () => (
  <ListView>
    <ListViewHeader title="Goods Receipt" />
