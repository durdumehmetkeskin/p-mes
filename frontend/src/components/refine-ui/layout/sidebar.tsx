...
/**
 * Drop menu entries the user has no read access to; a group/parent with no
 * visible children is hidden too. Ungated entries (not in the catalogue) stay.
 */
function filterMenu(
  items: TreeMenuItem[],
  state: AccessState,
  ...
  for (const item of items) {
    if (item.children && item.children.length > 0) {
      const children = filterMenu(item.children, state);
      if (children.length > 0) out.push({ ...item, children });
      continue;
    }
    if (canAccessResource(state, item.name)) out.push(item);
  }
  return out;
}
...
export function Sidebar() {
  ...
  // Until the access state loads, render nothing (avoids flashing entries the
  // user can't access). On a network failure the state fails open. Memoized so
  // the recursive tree walk doesn't re-run (and re-allocate child nodes, which
  // would defeat SidebarItem memoization) on every route-change re-render.
  const items = React.useMemo(
    () => (state ? filterMenu(menuItems, state) : []),
    [menuItems, state],
  );
