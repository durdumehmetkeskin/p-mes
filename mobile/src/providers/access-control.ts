...
// detail screen (warehouse responsibles operate tools from My Warehouse), not
// to browse the standalone Tools list.
...
/** Drop the cached permissions (call on login/register/logout). */
...
/** The full cached access state (used by the drawer nav filter). */
export function getAccessState(): Promise<AccessState> {
  return load();
}

/**
 * Whether the user may open a resource's area (read/list access) — the same rule
 * the accessControlProvider applies for the `list` action, but synchronous
 * against an already-loaded {@link AccessState}. Ungated resources (not in the
 * catalogue, e.g. dashboard/action pages) and admins are always allowed; fails
 * open when the catalogue lacks the key.
 */
export function canAccessResource(
  ...
  // The drawer/tab nav always asks for the list page.
  if (ADMIN_ONLY_LISTS.has(resource)) return false;
...
/**
 * Gates resources/actions by the user's effective permissions. A key not in the
 * catalogue is treated as ungated (dashboard, groups). Fails open on network
 * errors so users are never locked out by a hiccup.
 */
